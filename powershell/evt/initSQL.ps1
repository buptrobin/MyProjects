...
function CheckSnapin([string]$snapinName){
	Get-PSSnapin |where {$_.name -eq "SqlServerProviderSnapin100"}
}
...
$a=Get-PSSnapin |where {$_.name -eq "SqlServerProviderSnapin100"}
if(!$a){
...
Pop-Location
 Write-Host -ForegroundColor white 'SQL Server Powershell extensions are loaded.'
	Write-Host
